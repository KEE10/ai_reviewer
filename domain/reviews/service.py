...
class ReviewService:
    ...
    async def _fetch_pr_files(self, repo_fullname: str, pr_number: int) -> Dict[str, Any]:
...
Respond only with valid JSON, no additional text."""
    ...
    async def process_pr_review(self, pr_id: str, repo_fullname: str, review_id: int):
        ...
        await self.repository.update_review(review.id, status="completed", summary=summary, details=details)

        if comments:
            await self.repository.add_review_comments(review.id, comments)

        await emit("pr.reviewed", {"pr_id": pr_id, "status": "completed", "details": details})
