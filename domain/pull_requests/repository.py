...
class PullRequestRepository:
    ...
    async def get_pull_request(self, pull_request_id: str) -> PullRequestModel:
        pr = await self.session.execute(
            select(PullRequestModel).where(PullRequestModel.pull_request_id == pull_request_id)
        )
    ...
    async def get_pull_request_reviews(self, pull_request_id: str, page: int, limit: int, order: str, order_by: str):
        stmt = (
